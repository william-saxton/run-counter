...
export const persistence = {
  async loadActive(): Promise<Session | null> {
    ...
    const s = await store();
    return ((await s.get(activeKey())) ?? null) as Session | null;
  },

  async saveActive(session: Session | null): Promise<void> {
    if (!isTauri()) {
    ...
    const s = await store();
    if (session) await s.set(activeKey(), session);
    else await s.delete(activeKey());
  },
  ...
  async loadHistory(): Promise<Session[]> {
    ...
    const s = await store();
    return ((await s.get(historyKey())) ?? []) as Session[];
  },
